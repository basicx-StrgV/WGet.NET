...
## WinGetSourceManager ImportSource Methods
...
## WinGetSourceManager ExportSources Methods
...
it is recommended to now use the `GetInstalledSources` methods instead and create your own JSON from it.  
This is recommended, because the output provided by WinGet does not have a valid JSON format when multiple sources are exported.
